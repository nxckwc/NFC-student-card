...
const SettingsPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setFeedback({ type: 'success', message: t('attendanceSettingsSaved') })
      void queryClient.invalidateQueries({ queryKey: ['admin', 'settings'] })
    },
    onError: (error) => setFeedback({ type: 'error', message: getApiErrorMessage(error) ?? t('unableToSaveSettings') }),
  })
    ...
    <>
      ...
      <section className="max-w-2xl rounded-xl border border-border bg-surface p-5 shadow-[0_14px_36px_rgba(52,76,61,0.06)] sm:p-7">
        <div className="mb-6 flex items-center gap-3 border-b border-border-soft pb-5">
          ...
          <div>
            <h2 className="text-lg font-bold">{t('attendanceRules')}</h2>
            <p className="text-sm text-text-muted">{t('attendanceRulesDescription')}</p>
          </div>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <label className="text-xs font-bold uppercase tracking-wide text-text-muted" htmlFor="late-cutoff">
            ...
            <span className="mt-2 block text-xs font-normal normal-case text-text-faint">{t('lateCutoffDescription')}</span>
          </label>
          <label className="text-xs font-bold uppercase tracking-wide text-text-muted" htmlFor="school-timezone">
            ...
            <span className="mt-2 block text-xs font-normal normal-case text-text-faint">{t('schoolTimezoneDescription')}</span>
          </label>
        </div>
